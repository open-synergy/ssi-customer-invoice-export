...
> **Extends:** ssi_customer_invoice_export — model `customer_invoice_export`, aksi `01-create`
...
- **Access:** User is in group _Operating Unit_, under the _Customer Invoice Export_
  data ownership category, to see and set the **Operating Unit** field.
...
- **Operating Unit**: Many2one to the operating unit that owns this document. Only
  ...
  the document is in Draft. Scopes the invoices and journals selected by **Populate** to
  this operating unit; leave empty to keep **Populate** unrestricted by operating unit.
